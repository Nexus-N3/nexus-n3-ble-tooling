...
This directory contains the customer-facing Nexus N3 Dot host sample for the Nexus BLE gateway.

`stream_client.py` is the recommended starting point for customers who want to connect a supported gateway board, discover Nexus N3 Dot sensors, start streaming, and verify that data is flowing correctly.
...
- scans for Nexus N3 Dot sensors
- connects the requested number of sensors
- configures the sampling rate
- subscribes to the IMU streaming characteristic
- starts streaming
...
- at least one powered Nexus N3 Dot sensor advertising nearby
- Python 3
...
```bash
python NexusN3Dot/stream_client.py \
  --sensor-count 1 \
  --stream-seconds 10
```
...
To test at 50 Hz instead of the default 100 Hz:
